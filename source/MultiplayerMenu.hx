...
class MultiplayerMenu extends MusicBeatState
{
	...
	override function create()
	{
		...
		name.text = Storage.get("name", "You're name");
		Storage.set('name', name.text);
		...
		address = new FlxInputText(menuBG.width / 2 - 1000 / 1.75, 125, 1000, "example.com", 64);
		address.text = Storage.get("address", "example.com");
		Storage.set('address', address.text);
		address.callback = function(s1, s2)
		{
			Storage.set('address', s1);
		}
		add(address);
	...
	override function update(elapsed:Float)
	{
		...
		{
			if (++selection > menuText.length - 1)
				selection = 0;
		}
		...
		for (i in 0...menuText.length)
		{
			...
			{
				if (!selected)
					menuText[i].color = FlxColor.WHITE;
				else
					menuText[i].color = FlxColor.RED;
			}
		}
